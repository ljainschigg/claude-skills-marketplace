...
!!! warning
    ...
    **Manual Helm Chart Migration Required**  
    When mirroring images from MSR 2.x or MSR 3.x to MSR 4.x,
    Helm charts do not automatically migrate.  
...
4. Fill in the popup with the following details:
    ...
    !!! note
        ...
        Avoid specifying a user or repository namespace, as this will restrict access.  
        Using the root enables full crawling of the host.
...
7. In the popup window, review and confirm the following settings:
    ...
    - **Destination**: Make sure flattening is set to `Flatten 1 Level`.  
     If your environment uses an organization namespace in MSR 2 or MSR 3, you may choose an alternative flattening option.

    <details>
    <summary>Click to learn more about flattening options</summary>

    You can choose to flatten or retain the original structure of any organization or namespace.  
    Enabling the flattening option will merge all content into a single namespace (`ns`).  
    If your organization uses a more flexible namespace or organizational structure, review the following guidelines to understand how flattening may affect your setup:
...
8. Select the rule created in the previous step and click **Replicate**.  
   Be aware that pulling down the entire host may take some time to complete.
